...
export default function LoginPage(){
 ...
 async function google(){setError(''); const {error}=await supabase.auth.signInWithOAuth({provider:'google',options:{redirectTo:`${location.origin}/auth/callback?next=/dashboard`}}); if(error)setError('Google sign-in is not configured yet. Add the Google credentials in Supabase Auth settings.')}
 ...
 async function verify(e:React.FormEvent){e.preventDefault();setError('');const {error}=await supabase.auth.verifyOtp({phone,token:otp,type:'sms'});if(error)setError('That code is not valid anymore. Request a new one.');else location.href='/dashboard'}
