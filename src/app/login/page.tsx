...
export default function LoginPage(){
 ...
 const [phone,setPhone]=useState(''); const [sent,setSent]=useState(false); const [otp,setOtp]=useState(''); const [error,setError]=useState(''); const supabase=createClient()
 async function google(){setError(''); const {error}=await supabase.auth.signInWithOAuth({provider:'google',options:{redirectTo:`${location.origin}/auth/callback?next=${encodeURIComponent(next)}`}}); if(error)setError('Google sign-in is not configured yet. Add the Google credentials in Supabase Auth settings.')}
 async function sendOtp(e:React.FormEvent){e.preventDefault();setError(''); const {error}=await supabase.auth.signInWithOtp({phone});if(error)setError('We could not send the code. Check the number and try again.');else setSent(true)}
 async function verify(e:React.FormEvent){e.preventDefault();setError('');const {error}=await supabase.auth.verifyOtp({phone,token:otp,type:'sms'});if(error)setError('That code is not valid anymore. Request a new one.');else location.href=next}
